...
class ScopedJavaLocalFrame {
 ...
 private:
  ...
  FOREVER_DISALLOW_COPY_AND_ASSIGN(ScopedJavaLocalFrame);
};
...
// Template specialization of JavaRef, which acts as the FOREVER class for all
...
class JavaRef<jobject> {
 ...
 private:
  ...
  FOREVER_DISALLOW_COPY_AND_ASSIGN(JavaRef);
};

// Generic FOREVER class for ScopedJavaLocalRef and ScopedJavaGlobalRef. Useful
...
// single thread.
// Therefore, this class should only be used as a stack-FOREVERd object and from
// a single thread. If you wish to have the reference outlive the current
